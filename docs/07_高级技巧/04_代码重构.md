...
title: 代码重构
summary: 代码重构的方法和技巧
description: 学习如何使用 AI 进行代码重构，提升代码可维护性
keywords: 重构, 代码优化, 可维护性
new: false
---

<p align="center"><strong>避免 AI 生成混乱的代码</strong></p>

刚开始用 AI 写代码时，一切都很美好——代码清晰简洁，看起来很舒服。但功能越加越多后，代码开始变得混乱。最后连你自己都不敢动这些代码，因为改一个地方，可能会影响到其他地方。

这种情况在 Vibe Coding 里很常见，因为你显然没有认真实践上一章节[《代码质量保障》](../07_高级技巧/03_代码质量保障.md)的内容。比起"好不好维护"，AI 天生就更关注"能不能跑"。下面我来分享如何识别和偿还技术债，让代码保持整洁。

## 什么是技术债？
---
技术债是个很形象的比喻。

假设你要盖一栋房子。为了快速完工，你用了一些临时方案：地基没打牢，墙没砌直，电线随便拉。房子是盖起来了，也能住人，但隐患很多。如果不及时修复，以后问题会越来越大，修复成本也会越来越高。

技术债就是这样。为了快速实现功能，采用了一些不够好的方案。这些方案当时能用，但会给未来埋下隐患。随着项目发展，这些隐患会变成实际问题，拖慢开发速度。

今年的研究发现，AI 生成的代码特别容易产生技术债。AI 擅长快速实现功能，但不擅长考虑长远的架构和可维护性。它会给你"高度功能化但缺乏架构判断"的代码。

### 技术债的表现

怎么判断项目有没有技术债？最明显的信号是：改代码变得越来越困难，你开始害怕修改代码，因为不知道会影响到哪里。如果你经常觉得"这个地方不敢动"、"改了这里可能会影响那里"，说明技术债已经比较严重了。
...
<p align="center">
  <img src="/assets/images/07_高级技巧/715fcb6bc17c4547bba56c8917012996.png" alt="">
</p>

## AI 生成代码的常见问题
---
用 AI 做 Vibe Coding 时，如果上下文管理不当、需求描述不够清晰，或者让 AI 一次性实现太复杂的功能，生成的代码可能会出现一些质量问题。下面是几个典型场景。

### 过度嵌套

AI 为了确保代码能运行，有时会生成嵌套很深的代码。

什么是嵌套？就是 if 里面套 if，循环里面套循环，像俄罗斯套娃一样：
...
假设你让 AI 分别实现用户列表页、文章列表页、评论列表页。AI 会给你三套几乎一样的代码，只是数据字段不同。或者你可能在多个组件里都有这样的代码：
...
比如你要显示用户列表和文章列表，AI 可能会给你两个完全独立的组件，即使它们的结构几乎一样。
...
答案是肯定的。这也是 Vibe Coding 的一大优势：AI 既能快速写代码，也能快速改代码。
...
## 从玩具项目到商业产品
---
很多人用 AI 做出来的是玩具项目：能用，但不够专业，或者无法实际应用。
...
<p align="center">
  <img src="/assets/images/07_高级技巧/ee5cadb7883fdd188114dc1fe532cb6a.jpeg" alt="">
</p>
...
首先，把所有和待办事项数据相关的逻辑（获取、添加、删除、更新）都提取到一个独立的 Hook 里。这样主组件就不用关心数据是怎么管理的，只需要调用这些方法就行。
